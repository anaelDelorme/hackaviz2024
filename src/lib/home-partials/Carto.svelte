<script lang="ts">
  ...
  let timeCurrent = "2024-08-03 10:00:00+00:00";
 ...
 function getCityCoordinates(city: string, loc: any): [number, number] | null {
  ...
  if (loc && Array.isArray(loc.features)) {
    ...
    if (location) {
      console.log("YOOO");
    console.log("coord", location.geometry.coordinates[0]);
    ...
    if (!d3.select('#carto_spectateur').empty()) {
      ...
      Promise.all([
        ...
        svg.selectAll(".circle")
          ...
          .attr("r", d => (d.capacity / maxCapacity) * 14)
          .attr("class", "circle")
          .style("fill", "#FFF")
          .attr("stroke", "#FFF")
          .attr("stroke-width", 3)
...
function mousemove(event, d) {
  ...
  $: {
    const monTime = $horaires_store;

    updateMapOptions(); // Mettez à jour les options du graphique
  }
